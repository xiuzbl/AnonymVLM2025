...
class Qwen2VLAdaptGRPOTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        elif self.adaptive_strategy == "random":
            self.generation_config = GenerationConfig(
                max_new_tokens=self.max_completion_length,
                do_sample=True,  
                temperature=generation_temperature, # HACK
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            def generate_handle(inputs, use_cache=False):
                if use_cache:
                    # Disable gradient checkpointing for faster generation
            ...
            # we don't need unsquezee pixel_values
            ...
            if self.group_prefix is not None:
                ...
                if self.adaptive_strategy == "in_batch":
                    ...
                    grouped_prompts_text_to_generate = [random.choice(grouped_prompts_text)]
                for g in grouped_prompts_text_to_generate:
                    ...
                    # re-utilize the pixel_values and image_grid_thw to avoid additional memory usage
                    prompt_inputs_group["pixel_values"] = pixel_values
                    prompt_inputs_group["image_grid_thw"] = image_grid_thw
                    group_completion_ids.append(generate_handle(prompt_inputs_group, use_cache=self.use_cache_generation))
                    # get the prefix_mask
                    current_prefix_mask = torch.zeros_like(group_completion_ids[-1])
                    current_prefix_mask[:, :prompt_inputs_group['input_ids'].shape[1]] = 1
                    group_prefix_mask.append(current_prefix_mask)
                # concatenate the completions for each group
                if self.mask_prefix_for_loss:
                    assert len(self.group_prefix) == 1, "when mask prefix for loss calculation, only group num == 1 supported"
                    prompt_length = prompt_inputs_group['input_ids'].shape[1]
                prompt_completion_ids = pad_with_batch(group_completion_ids, padding_value=self.processing_class.pad_token_id)
                group_prefix_mask = pad_with_batch(group_prefix_mask, padding_value=0)[:, prompt_length:]
                prompt_ids = prompt_completion_ids[:, :prompt_length]
                completion_ids = prompt_completion_ids[:, prompt_length:]
                prompt_mask = prompt_mask.repeat_interleave(self.num_generations, dim=0)
            else:
                prompt_completion_ids = generate_handle(prompt_inputs, use_cache=self.use_cache_generation)
                # modified here to support use_cache
        ...
        if self.advantage_strategy == "default":
            ...
            assert self.group_num == 2, "this function currently only supports 2 groups"
            # split the rewards into multiple groups
            rewards = rewards_per_func.sum(dim=1)
