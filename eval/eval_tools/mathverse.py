...
def parse_options_from_question(question):
    opts = question.split("Choices:\n")[-1].split("\n")
    return [o[2:] for o in opts]

def mathverse_think_process_results(doc):
    ...
    for pred in [doc['prediction']]:
        ...
        if doc['question_type'] == "multi-choice":
            doc['options'] = parse_options_from_question(doc['question_for_eval'])
...
def CLEVR_eval(data, args):
    ...
    result_by_type = {'free-form':[], 'multi-choice':[]}
    item_to_correct = []
    for i in tqdm(range(len(data))):
        ...
        line_result = mathverse_think_process_results(line)
        if line_result['submission']['true_false']:
            class2res[line['problem_version']].append(1)
            correct_flag = True
            right += 1
        else:
            class2res[line['problem_version']].append(0)
        if content_match:
...
def main():
    ...
    parser.add_argument('--path', type=str, default='/data/MathVerse/')
    ...
    ds_meta = load_dataset(args.path, "testmini")['testmini']
    data = []
    res = model_output
